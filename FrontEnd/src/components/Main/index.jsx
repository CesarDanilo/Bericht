...
const Main = () => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        const newRelatorio = { id: generateRandomId(), ...formData };
        const updatedRelatorios = [...relatorios, newRelatorio];

        saveToLocalStorage("relatorios", updatedRelatorios);
        setRelatorios(updatedRelatorios);
        setIsDialogOpenAddRelatorio(false);
        setFormData({ empresa: '', descricao: '', dataInicial: '', dataFinal: '' });
    };
    ...
    const handleDelete = () => {
        const updatedRelatorios = relatorios.filter(item => item.id !== selectedCard.id);
        const updatedHistorico = [...historicoRelatorios, selectedCard];

        saveToLocalStorage("relatorios", updatedRelatorios);
        saveToLocalStorage("historicoRelatorios", updatedHistorico);

        setRelatorios(updatedRelatorios);
        setHistoricoRelatorios(updatedHistorico);
        setIsDialogOpen(false);
        setSelectedCard(null);
    };
    ...
    const handleDeleteHistory = () => {
        ...
        saveToLocalStorage("historicoRelatorios", []);
    };

    useEffect(() => {
        setRelatorios(loadFromLocalStorage("relatorios"));
        setHistoricoRelatorios(loadFromLocalStorage("historicoRelatorios"));
        setRelatoriosConcluidos(loadFromLocalStorage("relatoriosConcluidos"));
    }, []);
    ...
    return (
        <div className="flex flex-col lg:flex-row justify-center items-start min-h-screen p-10">
            <div className="flex flex-col md:flex-col w-full gap-4">
                <div className='flex w-full gap-4'>
                    ...
                    {historicoRelatorios.length > 0 && (
                        <div className="flex flex-col bg-[#191919] rounded-lg p-4 space-y-4 md:w-[20%] max-h-[384px] overflow-y-auto scrollbar-hidden">
                            <h3 className="text-white font-bold text-lg">Histórico</h3>
                            <button className="text-primary" onClick={handleDeleteHistory}>Limpar Histórico</button>
                            {historicoRelatorios.map(item => (
                                <Cards key={item.id} empresa={item.empresa} dataFinal={item.dataFinal} />
                            ))}
                        </div>
                    )}
                </div>
            ...
            <div className="bg-[#191919] rounded-lg w-full md:w-[680px] h-96 p-6 shadow-md">
                <div className="flex justify-between items-center scrollbar-hidden">
                    <h3 className="text-white font-bold text-lg">Relatórios Pendentes</h3>
                    <button className="flex justify-center items-center rounded-full bg-primary w-8 h-8" onClick={() => setIsDialogOpenAddRelatorio(true)}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-black">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                        </svg>
                    </button>
            ...
            {isDialogOpenAddRelatorio && (
                <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
                    <div className="bg-[#191919] rounded-lg p-6 w-3/4 shadow-lg">
                        <h3 className="text-white font-bold text-xl mb-4">Cadastrar Novo Relatório</h3>
                        <form onSubmit={handleSubmit}>
                            <input type="text" name="empresa" placeholder="Nome da Empresa" value={formData.empresa} onChange={handleFormChange} className="w-full p-2 mb-2 rounded bg-custom-black text-neutral-200" required />
                            <textarea name="descricao" placeholder="Descrição" value={formData.descricao} onChange={handleFormChange} className="w-full p-2 mb-2 rounded bg-custom-black text-neutral-200" required />
                            <input type="date" name="dataInicial" value={formData.dataInicial} onChange={handleFormChange} className="w-full p-2 mb-2 rounded bg-custom-black text-neutral-200" required />
                            <input type="date" name="dataFinal" value={formData.dataFinal} onChange={handleFormChange} className="w-full p-2 mb-2 rounded bg-custom-black text-neutral-200" required />
                            <div className="flex justify-between mt-4">
                                <button type="button" className="bg-custom-black text-neutral-500 rounded px-4 py-2" onClick={() => setIsDialogOpenAddRelatorio(false)}>Cancelar</button>
                                <button type="submit" className="bg-primary text-neutral-900 rounded px-4 py-2">Cadastrar</button>
                            </div>
            ...
            {isDialogOpen && selectedCard && (
                <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
                    <div className="bg-[#191919] rounded-lg p-6 w-full max-w-[768px] shadow-lg">
                        ...
                        <div className='flex gap-80'>
                            <button className='bg-red-600 w-16 h-7 text-neutral-200 rounded-lg' onClick={handleDelete}>Excluir</button>
                            <button className='bg-primary w-16 h-7 rounded-lg' onClick={() => setIsDialogOpen(false)}>Sair</button>
                        </div>
                    </div>
                </div>
            )}
